...
// Função para gerenciar a fila de jogadores
void gerenciarFila(FilaJogadores *fila)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        // Título da interface de fila
        DesenharTexto("GERENCIAR FILA DE JOGADORES", LARGURA_TELA / 2 - 200, 100, 30, WHITE);

        // Opção do menu de fila
        const char *opcoes[] = {
        ...
        {
            DesenharTexto(opcoes[0], LARGURA_TELA / 2 - 150, 200, 20, YELLOW);
        }
        else
        {
            DesenharTexto(opcoes[0], LARGURA_TELA / 2 - 150, 200, 20, WHITE);
        }
        ...
        // Campo para digitação do nome (em um retângulo centralizado)
        if (digitandoNome)
        {
            int boxWidth = 400;
            int boxHeight = 100;
            int boxX = (LARGURA_TELA - boxWidth) / 2;
            int boxY = 260;
            DrawRectangle(boxX, boxY, boxWidth, boxHeight, DARKGRAY);
            DesenharTexto("Digite o nome do jogador:", boxX + 20, boxY + 20, 20, WHITE);
            DesenharTexto(nome, boxX + 20, boxY + 50, 20, WHITE);
        ...
        // Mostra informações da fila
        int numJogadoresFila = tamanhoFila(fila);
        DesenharTextoFormatado(TextFormat("Jogadores na fila: %d", numJogadoresFila),
                               20, ALTURA_TELA - 40, 20, YELLOW);

        // Mostra a lista de jogadores na fila (abaixo do campo de digitação)
        if (numJogadoresFila > 0)
        {
            int listaY = 400;
            DesenharTexto("Jogadores na fila:", LARGURA_TELA / 2 - 150, listaY, 20, WHITE);
            NoJogador *atual = fila->inicio;
            int i = 0;
            while (atual != NULL && i < 5)
            { // Mostra até 5 jogadores
                DesenharTextoFormatado(TextFormat("%d. %s", i + 1, atual->nome),
                                       LARGURA_TELA / 2 - 150, listaY + 30 + i * 25, 20, WHITE);
                atual = atual->prox;
                i++;
            }
            if (numJogadoresFila > 5)
            {
                DesenharTexto("...", LARGURA_TELA / 2 - 150, listaY + 30 + 5 * 25, 20, WHITE);
            }
        }

        // Instrução para sair
        DesenharTexto("Para sair, pressione ESC", LARGURA_TELA / 2 - 150, ALTURA_TELA - 50, 20, WHITE);
